...
알려진 독소 예문과 유사한 문구를 찾은 **추가 검토 후보**이며 위법·불공정 판정이 아닙니다. 빈 목록도
"독소 없음"을 보장하지 않고, 현재 패턴 검색에서 신호를 찾지 못했다는 뜻으로만 해석해야 합니다.
현재 v5 held-out 독소 이진 평가는 P 0.571·R 0.500·F1 0.533이며 패턴 종류의 정확성까지 나타내지는
않습니다. 자세한 한계는 [v5 품질 리뷰](docs/quality/v5_review.md)를 참고하세요.
...
- **양방향 검토 신호:** 표준 대비 상태와 독립된, 알려진 독소 패턴 유사 신호를 제공합니다.
