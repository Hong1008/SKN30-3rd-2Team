...
sequenceDiagram
    ...
    외부 클라이언트->>mcp 서버: review_contract(contract_type, file_content, file_name)
    ...
    review_pipe->>koreanLaw 어댑터: 정적 매핑이 있는 MISSING 조항 → 법령 근거 조회
    review_pipe-->>mcp 서버: List[DeviationResult]
    mcp 서버-->>외부 클라이언트: ReviewContractResponse(status=OK, results)
```
...
4. **법령 근거 부착**: 정적 매핑이 있는 `MISSING` 조항만 `korean-law-mcp`로 질의해 근거 조문(`GroundingLaw`)을 조건부로 붙인다. `NONE`/`EXTRA`/`NO_MATCH`는 조회하지 않으며 필요한 근거는 `get_category_grounding`으로 별도 조회한다.
...
| `review_contract` | 계약서 전체 → 파싱→매칭→분류→누락·주의 문구 탐지, 일부 `MISSING`에 조건부 법령근거 (`async`, progress 보고) | 전체(`MISSING` 포함) |
