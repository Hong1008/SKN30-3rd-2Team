...
- **1차 MCP:** `review_contract_candidates`가 법령 조회 없이 `MISSING` / `EXTRA` / `NONE` / `NO_MATCH` 신호, 대응 표준조항과 독소 패턴을 반환한다. 필요한 법령은 `get_grounding`으로 별도 조회한다. 기존 `review_contract`는 일부 `MISSING`에 조건부 근거를 붙이는 호환 경로다.
...
현재 활성 개발 작업은 없다. 1차 MCP는 외부 클라이언트가 `review_contract_candidates`, `get_grounding`,
