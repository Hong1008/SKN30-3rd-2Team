...
> 기획서 8장 · 결정 로그 [07-01 §12~](../decisions/07-01.md) · 필독 [src/eval/README.md](../../src/eval/README.md)
...
## 현재 상태 (2026-07-10 실측 반영)

> **현재 판정: 평가 하니스와 Track A/B 실행은 완료.** A-1 hybrid rerank 5:5는 Recall@5=0.920·MRR=0.903,
> 이탈 F1=0.759, 독소 F1=0.281이다. 임계값 후보가 held-out에서 재현되지 않아 기본값을 유지한다.
> 남은 일은 독소 top-3와 이탈 매칭 근거를 남기는 **case-level 진단 덤프**다.
...
| 통합 실행 리포트(`main()`) | ✅ 구현·실측 완료(2026-07-10) |
| 트랙 B(M:N 표준 커버리지·강건성) | ✅ 구현·Golden E2E 실측 완료(지원범위 판별 포함) |

**골든셋 분포 (v3 - 총 117건):** deviation = NONE 75 (순수 33, 변형 42) / EXTRA 42 (독소 12, 정상 30) / **MISSING 0**,
trap = none 63 (순수 NONE 33 + 신규 정상 EXTRA 30) / paraphrase 19 / partial 7 / negation 7 / party 4 / contradiction 3 / number 5 / reorder 3 (trap 다각화 수립),
...
### 케이스 필드 (조항 단위 — 트랙 A) — [예시](../../src/eval/golden/v3_sw_freelance.json)
| 필드 | 의미 |
| --- | --- |
...
| `gold_clause_id` | 정답 표준조항 id. **비표준(EXTRA)이면 `null`**. 버전 구분을 위해 `{contract_type}-{version}-art{N}` 명명 규약 준수 (예: `si_subcontract-2025-art1`) |
| `gold_deviation` | `NONE` / `EXTRA` (CHANGED 폐지됨. 트랙 A엔 `MISSING` 없음) |
...
`contradiction`(표준 조항번호를 언급하나 내용은 반대) / `negation`(부정어 반전) / `party`(당사자 이탈) / `number`(수치/기한 오염).
...
- [x] 트랙 A 전체 통합 실행·ablation·RRF 가중치 스윕 실측
- [x] match/toxic threshold tuning·held-out 보정(개선 미재현으로 기본값 유지)
- [x] **LLM-judge 금지** — 결정론적 계산만 (AGENTS.md 규칙 #5)
- [x] 트랙 B/Golden E2E 실행 및 강건성·지원범위 확인
- [x] 독소 top-3 pattern/id/score 및 이탈 matched standard/version/category의 case-level 진단 덤프 — `vN_diagnostics.json`·`vN_diagnostics.md` 자동 생성
  - [x] 실제 review 실행 후보 trace 정합성 검증 및 v3 FP/FN 수동 대조 — `golden/v3_diagnostics_review.md`

## 참고
- 📖 [src/eval/README.md](../../src/eval/README.md) — 평가 철학·골든셋 포맷·지표별 사용법 (필독)
- 결정 배경: [07-01 결정 로그](../decisions/07-01.md) (매칭 불변식·ablation 스위치), 기획서 8장. 트랙 B 스코프 확정(라벨리스 M:N 표준 커버리지 + 강건성 스팟체크)은 본 문서 §트랙 B 가 단일 진실원.
