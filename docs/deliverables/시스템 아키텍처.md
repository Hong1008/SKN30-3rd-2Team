...
현행 운영 구성은 MCP 컨테이너와, 무거운 ML 연산을 분리해 외부에 두는 RunPod 서버리스 워커로 구성된다.
`demo/` Streamlit 소스와 Docker Compose 번들은 deprecated되어 배포 대상에서 제외됐다.
...
`mcp` 컨테이너의 `Dockerfile`은 `uv sync --frozen --no-dev`로 `sentence-transformers` 등 dev 그룹(로컬 모델)을 **제외**하고 빌드하므로, 운영 이미지에서는 반드시 `APP_ENV=prod` + `RUNPOD_SERVERLESS_API_KEY`/`RUNPOD_ENDPOINT_ID`가 필요하다.
...
core(순수함수)     adapter(외부 I/O)   ← ports 구현: db·vector·embedder·reranker·kordoc·koreanLaw
   ...
   └──── pipe(조립) ────┘            ← 오프라인 준비 + 런타임 review_contract
                  ▲
            mcp_server                ← FastMCP 노출
```

### 3.1 contracts — 동결 계약 ([src/contracts/](../src/contracts/))
- `enums.py`: 닫힌 값 집합. `ContractType`(`SW_FREELANCE`/`SI_SUBCONTRACT`/`SM_SUBCONTRACT`/`SW_EMPLOYMENT`), `Category`(`PAYMENT`/`IP_OWNERSHIP`/`SCOPE_SOW`/`CONTRACT_PERIOD`/`TERMINATION`/`CONFIDENTIALITY`/`LIABILITY`/`DISPUTE`/`SOCIAL_INSURANCE`/…), `Deviation`(`MISSING`/`EXTRA`/`NONE`/`NO_MATCH`), `ToxicPattern`, `ProgressPhase`. 여기 없는 문자열 값은 코드 어디에도 직접 쓰지 않는다.
...
| 함수 | 단계 | 반환 |
| --- | --- | --- |
| `select_best_match(candidates)` | 리랭커 직후 최고 후보 선택 | `(StandardClause \| None, float)` |
| `classify_clause_deviation(...)` | 후보 유무와 단일 매칭 임계값으로 `EXTRA`/`NONE` 분류 | `Deviation` |
...
`CHANGED`, `change_threshold`, 본문 유사도 2차 판정은 현행 1차에서 제거되었다. 현재 1차는 재정렬
점수가 단일 `match_threshold` 이상이면 잠정 `NONE`, 미만이면 `EXTRA`로 분류하며 의미 차이 해석은
서버 밖 2차 계층의 책임이다.
...
### 4.2 런타임: 계약서 전체 검토 (`review_contract_candidates` 권장)
```mermaid
sequenceDiagram
    autonumber
    외부 클라이언트->>mcp 서버: 계약서 업로드 (file_content base64 또는 file_path)
    외부 클라이언트->>mcp 서버: assess_contract_scope(...)
    mcp 서버-->>외부 클라이언트: 지원 범위·계약 유형 후보
    외부 클라이언트->>mcp 서버: review_contract_candidates(contract_type, file_content, file_name)
    ...
    mcp 서버->>mcp 서버: 공개 DTO로 변환·결과 방향 분리
    mcp 서버-->>외부 클라이언트: ReviewContractCandidatesResponse
    opt 특정 조항의 법령 원문이 필요함
        외부 클라이언트->>mcp 서버: get_category_grounding(category, contract_type)
    end
```
...
3. **최적 매칭 & 이탈 분류**: `select_best_match` → `classify_clause_deviation`으로 `EXTRA`/`NONE`을 분류하고, 루프 종료 후 `detect_missing_clauses`로 `MISSING`을 찾는다. 검색 후보 자체가 없으면 pipe가 `NO_MATCH`를 직접 채운다.
4. **법령 근거 분리**: 권장 도구 `review_contract_candidates`는 법령을 조회하지 않는다. 필요한 조항만 `get_category_grounding`으로 별도 조회한다. 호환 도구 `review_contract`는 정적 매핑이 있는 일부 `MISSING`에만 조건부 근거를 붙인다.
...
전체 계약서가 아니라 조항 하나만 볼 때는 `match_clause`(후보 나열만, 판정 없음) 또는 `classify_clause_candidate`(재정렬→매칭→판정까지)를 쓴다. `classify_clause_candidate`는 법령 조회를 하지 않고 응답에도 `grounding`이 없다. `MISSING`은 표준조항이 계약서 전체에 없다는 뜻이라 단일 조항 도구에서는 반환할 수 없다.
...
| 도구/리소스 | 역할 | 이탈 판정 |
| --- | --- | --- |
| `parse_contract_clauses` | 계약서 → 공개 조항(`PublicClause[]`) 분해 | 없음 |
| `parse_contract` | 계약서 → 도메인 조항(`Clause[]`) 분해(호환 도구) | 없음 |
| `match_clause` | 단일 조항 텍스트 → 유사 표준조항 후보 나열 | 없음 (검색 전용) |
| `classify_clause_candidate` | 단일 조항 → 재정렬·매칭·판정, 법령 필드 없음 | `EXTRA`/`NONE`/`NO_MATCH` |
| `classify_clause` | 기존 단일 조항 호환 도구. 항상 빈 `grounding` 유지 | `EXTRA`/`NONE`/`NO_MATCH` |
| `review_contract_candidates` | 법령 조회 없이 사용자 조항 결과와 `MISSING` 표준조항을 분리해 반환 | 전체(`MISSING` 포함) |
| `review_contract` | 계약서 전체 → 파싱→매칭→분류→누락·주의 문구 탐지, 일부 `MISSING`에 조건부 법령근거를 포함하는 호환 도구 (`async`, progress 보고) | 전체(`MISSING` 포함) |
| `get_category_grounding` | 카테고리 → 관련 법령 조문과 명시 상태. 미매핑·결과 없음·외부 오류·시간 초과 구분 | 없음 (근거 조회) |
| `get_grounding` | 카테고리 또는 법령명이 명시된 질의 → 관련 법령 조문(호환 도구) | 없음 (근거 조회) |
...
`standard://...` 리소스는 표준조항을 독립적으로 탐색하거나 `clause_id`로 재조회할
때 사용한다. 전체 검토 응답에 대응 표준조항 본문이 있으므로 후속으로 항상 호출하지 않는다.
...
| 운영(prod) | RunPod Serverless GPU 워커 (`RUNPOD_SERVERLESS_API_KEY`/`RUNPOD_ENDPOINT_ID`) | `mcp` 이미지에 `data/03_normalized`·`data/migration` 스키마 내장, 컨테이너 내 SQLite/Chroma 재생성 | 콜드스타트 최소화, on-demand GPU 과금(`just embed-on`/`embed-off`) |
...
- **UX 프레이밍**: 모든 도구의 docstring이 반환값을 "검토 후보"로 규정하며, `위법`/`불리함`/`승소` 같은 단정적 결론을 생성하지 않도록 명시한다 (규칙 #3). 법령 조회 도구는 조문만 반환하고 해석을 덧붙이지 않는다.
