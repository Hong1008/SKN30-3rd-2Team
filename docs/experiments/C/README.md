...
C1은 tuning을 통과했으나, 승인된 held-out에서 FP가 1건에서 2건으로 증가해 채택 기준을
충족하지 못했다. 따라서 **보류**이며 기본 런타임 동작으로 채택하지 않는다. held-out 단회 실행은
완료됐으므로 같은 split을 다시 실행하거나, 그 결과를 보고 C1을 조정하지 않는다. 상세 판정은
`docs/dicision/07-13.md`의 “실험 C1 최종 판정”을 따른다.
