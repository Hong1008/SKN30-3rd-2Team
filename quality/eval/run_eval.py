...
Driver(트랙 A, 통합/수동 실행 — 단위테스트 밖): 골든셋 fixture의 user_clause 를
...
GOLDEN_DIR = "quality/fixtures/legacy/golden"
V5_GOLDEN_DIR = "quality/fixtures/v5"
EXPERIMENT_S_DIR = Path("docs/quality/experiments/S")
...
def _load_golden(version: str, golden_dir: str = GOLDEN_DIR) -> List[Dict]:
    ...
    if version == "v5" and golden_dir == GOLDEN_DIR:
        golden_dir = V5_GOLDEN_DIR
    golden: List[Dict] = []
...
def _write_result_md(
    ...
    lines = [
        f"# {version} — 평가 결과",
        "",
        f"> 자동 생성: `quality/eval/run_eval.py` · {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} "
...
EXPERIMENT_C_DIR = Path("docs/quality/experiments/C")
...
GOLDEN_B_DIR = "quality/fixtures/track_b/golden_b"
