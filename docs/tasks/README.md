...
WorkShield 작업 카드 색인입니다. 상태 기준은 **2026-07-11 결정 로그와 현재 구현**입니다.

> 기준: [2026-07-11 결정 로그](../dicision/07-11.md). `X_graph.md`와 `Z_Lever.md`는 기각·제외된
...
| D | ✅ 평가 하니스·Track A/B·v3/v4/v5 실측 및 case-level 진단 완료 | v5 이탈 기준선·held-out 동결, 후속 변경안만 별도 검증 |
...
| L | ✅ v3 완료·v4 challenge·v5 이탈 골든 완료 | v5 기준선에서 한 오류 메커니즘씩 별도 실험 |
| M | ⏸️ K 이후 재검토 | K 출력 계약 전 평가 구현은 진행하지 않음 |
| N | ✅ 코퍼스 40→90·v3/v4 진단 및 정합성 정리 완료 | 개선안은 사람 사인오프 후 별도 카드로 진행 |
...
## 🔧 재설계 트랙 (07-07 세션 — 현행 NONE/EXTRA 1차 판정과 2차 경계)
...
| 카드 | 내용 | 선행 |
| --- | --- | --- |
| [J_deviation_redesign.md](J_deviation_redesign.md) | 과거 CHANGED/coverage 설계 폐기 후 단일 매칭 게이트로 단순화. 현행 1차는 잠정 NONE/EXTRA/NO_MATCH와 계약 단위 MISSING | 없음 |
| [K_second_stage_llm.md](K_second_stage_llm.md) | 잠정 NONE과 EXTRA의 의미 차이·추가조항 성격 설명 (서버 밖 LLM) | J |
