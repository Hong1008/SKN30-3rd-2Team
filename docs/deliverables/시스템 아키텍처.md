...
4. **법령 근거 부착**: 정적 매핑이 있는 `MISSING` 조항만 `korean-law-mcp`로 질의해 근거 조문(`GroundingLaw`)을 조건부로 붙인다. `NONE`/`EXTRA`/`NO_MATCH`는 조회하지 않으며 필요한 근거는 `get_grounding`으로 별도 조회한다.
...
| `get_grounding` | 카테고리 또는 법령명이 명시된 질의 → 관련 법령 조문 | 없음 (근거 조회) |
