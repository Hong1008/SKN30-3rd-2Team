...
# "아니된"(된=되+ㄴ)은 "아니되"와 다른 유니코드 문자라 별도 패턴 필요 — "~하여서는 아니 된다"는
# 계약서에서 가장 흔한 금지 표현인데 "아니되"만으로는 매칭되지 않았다(v3 골든 작성 중 실측 발견).
_NEGATION_RE = re.compile(r"아니하|아니한|아니되|아니된|않|못한다|못하")
...
# 숫자도 앞 문자 제외 대상에 포함 — "10000분의 1을"처럼 숫자 뒤 목적격 조사 '을'이
# 당사자로 오탐되던 문제(v3 골든 작성 중 실측 발견. 앞이 한글이 아니기만 하면 통과되던 허점).
_PARTY_RE = re.compile(
    r"수급사업자|원사업자|도급인|수급인|위탁자|수탁자|발주자"
    r"|(?<![가-힣0-9])[갑을](?=[은이가의와과에]|[^가-힣]|$)"
)
