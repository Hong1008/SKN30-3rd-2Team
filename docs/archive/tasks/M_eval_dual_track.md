# [재설계 M — 과거 평가 초안] eval — 1차/2차 분리 채점 + 결정론 규칙(#5)과의 공존

> **현재 상태: 보류된 이력.** 아래 내용은 `CHANGED`와 coverage 2-pass가 남아 있던 전제를 사용한다. 현행 1차 출력은 `NONE`/`EXTRA`/`NO_MATCH`와 계약 단위 `MISSING`이며, K의 structured output 계약이 확정된 뒤 새 평가 카드로 재설계한다. 현재 구현·평가 기준으로 사용하지 않는다.

> **현재 상태: K 이후 재검토.** K의 structured output 계약과 모델이 미확정이므로 질문과 지표 코드를
> 먼저 구현하지 않는다. 현재 1차 평가는 [D_eval.md](D_eval.md)의 결정론적 지표를 유지한다.

> 근거: [07-03 결정 로그](../decisions/07-03.md) §3 미해결항목, 07-07 세션(1차/2차 판정 경계 설계).
...
- [x] **비율 스윕 실행** — 2026-07-10 v3 전체에서 5:5·7:3·8:2·9:1을 비교했다. 5:5 rerank와
      7:3은 Recall@5 0.920·MRR 0.903으로 동률이고, 8:2·9:1은 Recall@5 0.907로 하락했다.
- [ ] **RRF held-out 검증** — 현재 held-out은 threshold 보정용이다. RRF 후보 선택을 위한 독립 검증은
      별도 골든 확장 때 수행한다.
- [x] **현행값 유지 기록** — 유의미한 우위가 없으므로 `review_contract`의 기본 동일가중 5:5를
      그대로 유지한다. 가중치 배선·기본값 변경은 하지 않는다(2026-07-10).
