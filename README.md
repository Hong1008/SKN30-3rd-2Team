...
> IT·SW 분야의 도급·하도급 계약서를 표준계약서와 조항별로 비교하고, 필요할 때 관련 법령 원문을 별도로 조회할 수 있는 MCP 서버
...
이 저장소는 웹 화면이 아닌 **MCP 서버**를 제공합니다. MCP 클라이언트는 검토 응답에 포함된
표준조항을 바로 사용하고, 특정 조항의 법령 원문이 필요할 때 `get_category_grounding` 등 별도 조회 도구를
호출해 사용자 경험과 자연어 설명을 구성할 수 있습니다.
...
| 제공 결과 | 조항별 대응 상태, 누락 가능성, 주의 문구 유사 신호, 대응 표준조항, 필요할 때 별도 조회하는 법령 원문 |
...
3. 클라이언트는 사용자 조항, 표준조항과 주의 문구 신호를 사용하고, 필요한 조항만 `get_category_grounding`으로 법령 원문을 조회합니다.

전체 계약서가 아닌 조항 하나만 비교할 때는 `parse_contract_clauses`로 공개 조항 DTO를
받은 뒤, 필요한 조항을 `classify_clause_candidate`로 분류하는 부분 흐름을 사용합니다.
`parse_contract`, `review_contract`, `get_grounding`, `classify_clause`는 기존 응답을 유지하는
호환 경로이며 신규 클라이언트의 기본 경로가 아닙니다.
...
계약서 전체의 누락 가능성·주의 문구 신호 확인
    ↓
사용자 조항·대응 표준조항이 포함된 MCP 응답
    ↓ 필요한 경우에만
카테고리로 관련 법령 조회(미매핑·결과 없음·외부 오류·시간 초과 구분)
```
...
`SW_FREELANCE` 표준계약서와 비교했습니다. 아래는 2026-07-13 현재 `prod` 구성에서 **호환
도구 `review_contract`를 실행한 기록**입니다. 신규 클라이언트는 동일한 검토 파이프라인의
공개 DTO 결과를 반환하는 `review_contract_candidates`를 사용합니다.
...
발췌한 것입니다. 표준조항 전체 본문과 `MISSING` 일부에 조건부로 부착되는 법령 원문은
가독성을 위해 생략했습니다.
...
- **법령 원문 연결:** 권장 전체 검토는 법령을 조회하지 않고, 필요한 조항만 `get_category_grounding`으로 조회합니다. 호환 도구 `review_contract`는 일부 `MISSING`에만 정적 근거를 조건부로 붙입니다.
...
| 도구 | 역할 |
| --- | --- |
| `get_mcp_capabilities` | 권장 워크플로우와 기존 호환 도구의 대체 관계를 구조화해 반환 |
...
| `review_contract` | 일부 `MISSING`의 조건부 근거까지 포함하는 기존 전체 검토 호환 도구 |
| `parse_contract_clauses` | 계약서를 도메인 모델과 분리된 공개 조항 목록으로 반환하는 권장 파싱 도구 |
| `parse_contract` | 내부 `Clause` 스키마를 유지하는 기존 파싱 호환 도구 |
...
| `legal_research` / `legal_analysis` 등 | 같은 MCP에 노출된 외부 `korean-law-mcp` 프록시. 반환 내용은 법률적 결론이 아닌 참고 자료 |

새 클라이언트는 `review_contract_candidates`를 기본 전체 검토 도구로 사용하세요. 기존
`review_contract`는 `grounding`을 포함하는 호환 경로로 유지됩니다.

`standard://{contract_type}`과 `standard://{contract_type}/{clause_id}`는 표준조항을 독립적으로
탐색하거나 저장한 ID로 원문을 다시 열 때 사용하는 리소스입니다. 검토 응답에 대응
표준조항 본문이 이미 있으므로 전체 검토 뒤 항상 추가 호출하지 않습니다.
