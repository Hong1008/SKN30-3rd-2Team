...
## 완료 조건 (DoD)
- [x] §승인 게이트 3건 확정
- [x] `progress_callback=None` 회귀: 기존 파이프 테스트 15건 그대로 통과
- [x] `progressToken` 미지원 클라이언트에서 `review_contract` 정상 결과 반환
- [x] `progressToken` 지원 시 조항별 progress notification 수신 확인
- [x] law MCP 어댑터 중첩 루프 충돌 없음 검증
