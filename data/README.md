...
| `03_normalized/archive/` | 구버전 표준조항·서브청크 이력. Git에는 보관하지만 SQLite·Chroma 활성 코퍼스에는 적재하지 않음 | ✅ 커밋 |
...
| `chroma/` | **생성물.** Chroma 벡터 인덱스(`chroma.sqlite3`, `<uuid>/` 세그먼트 폴더). RDB와 생명주기가 달라 `migration/`과 분리된 전용 폴더. `just build-index` 실행 시 폴더 전체를 삭제 후 재생성(고아 폴더 증식 방지) | ❌ gitignore |
...
현재 **활성** 계약 종류·버전: `si_subcontract`(2025) · `sm_subcontract`(2025) ·
`sw_employment`(2020) · `sw_freelance`(2020). SI·SM 2022판은 `archive/`에 보관한다.

> 활성 루트에는 계약유형별로 하나의 version만 둘 수 있다. `just migrate`는 복수 version을 발견하면
> 실패한다. 새 표준판을 추가할 때는 직전 활성 JSON(조항·서브청크)을 `archive/`로 옮겨 검색 풀이
> 혼재되지 않게 한다.
