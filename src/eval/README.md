...
**임계값 스윕:** 트랙 A 결과에는 전체 골든 기준 `match_threshold`(`0.40`~`0.60`) 및
`toxic_threshold`(`0.60`~`0.90`)별 P/R·특이도·정확도·F1·혼동행렬도 함께 기록됩니다. 이는 후보를
비교하는 용도이며, 동일 골든셋의 최고값을 자동으로 프로덕션 기본값에 반영하지 않습니다. 채택 전에는
별도 held-out 검증과 사람 사인오프가 필요합니다.
