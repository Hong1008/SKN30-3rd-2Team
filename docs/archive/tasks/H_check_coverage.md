...
> **상태: 폐기 / 현 상태로 수행 금지 (2026-07-11 정리).** 이 카드는 J 이전의
> `Deviation.CHANGED` 상향과 `uncovered_sub_chunk_ids` 노출을 전제로 한다. 현재 1차 판정 계약에서
> 해당 전제가 제거됐으므로 아래 내용은 과거 설계 기록으로만 보존한다. 다시 필요하면 동결 계약을
> 임의 복원하지 말고 새 카드와 사람 사인오프로 재설계한다. G의 부모 roll-up 완료와는 별개다.
