...
from core import has_conflicting_parent_candidates, select_best_match
...
def test_표준조항과_서브청크부모가_다르면_충돌이다():
    assert has_conflicting_parent_candidates(_std("standard"), _std("sub-parent"))
    assert not has_conflicting_parent_candidates(_std("same"), _std("same"))
    assert not has_conflicting_parent_candidates(_std("standard"), None)
