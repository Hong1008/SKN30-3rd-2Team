...
### 2026-07-12: P0-3 법령 앵커 재검증

- 법령 식별자와 최신 시행 버전을 `korean-law`로 재조회했다.
- `v4_legal_anchors.md`의 11개 법령 인용을 `legal_analysis --mode verify_citations`로 검증했다.
  - 성공 11건, 오류 0건, 확인필요 0건.
- 조회 응답·식별 정보는 [v4_legal_anchors_evidence.md](v4_legal_anchors_evidence.md)에 보존했다.
  - API 키·인증 정보·환경변수·토큰 등 비밀값은 기록하지 않았다.
- 앵커별 적용 가능성 판정을 추가했다. 하도급법은 적용 요건 검토 필요, 특허법 제99조는 특허권 공유에 한정, 부정경쟁방지법 제2조는 영업비밀 요건에 한정한다.
- 법령 검증으로 `gold_toxic` 라벨과 split은 변경하지 않았다.
- `v4-sw-18`은 이미 당사자 합의 요율·기간·상한 문구로 정정되어 있어 추가 골든 변경은 하지 않았다. case ID·split·threshold는 유지한다.
- 동일 설정 `just eval a v4 prod` 재실행은 RunPod 임베딩 작업이 `IN_QUEUE`로 종료되어 새 결과 산출물 교체까지 진행하지 못했다. 기존 `v4_result.md`와 진단 산출물은 이 실행 실패 결과로 교체하지 않는다.
...
### 2026-07-12: P0-4~P0-8 작업 반영 (사인오프 대기)

- `v4-sw-18` 승인 문구: “당사자가 합의한 지체상금 요율과 적용 기간 및 한도를 계약서에 명시하고, 실제 지연 일수에 비례하여 산정한다.”
- 기간·IP·관할 설명은 법적 결론이 아닌 사람이 대조할 검토 후보로 유지한다.
- `gold_toxic`, `gold_deviation`, `gold_clause_id`, case ID, split 및 임계값은 변경하지 않았다.
- v4 용도는 독소 hard-negative challenge set으로 고정한다. 총 54건(튜닝 36건, held-out 18건), `EXTRA` 6건은 모두 SM에 있다.
- 독소 held-out 결과 TP=2, FP=0, FN=7, TN=9, precision=1.000, recall=0.222, F1=0.364는 성능 개선으로 해석하지 않는다. SI·SW 이탈 지표는 유형 비교에 사용하지 않는다.
- P0-6에서는 이 문서와 작업 카드만 해석 한계를 동기화한다. `v4_result.md`와 진단 산출물은 P1 재평가 결과로만 교체한다.
- 승인된 최종 골든을 기준으로 `tests/eval/fixtures/v4_content_snapshot.v4.json`을 생성했다. NFC, 줄바꿈 통일, 앞뒤 공백 제거, 연속 공백 축소만 적용하며 문장부호·부정·수치·기간 표현은 보존한다.
- P0 정적 검증: 콘텐츠 스냅샷·불변성·held-out·골든 검증·평가 하니스 테스트 총 29건 통과.
- 네 역할의 담당자·승인일이 기록되지 않아 P0-4 사인오프 상태는 대기 중이다.
- P1 `just eval a v4 prod`는 2026-07-12 20:36:25에 `APP_ENV=prod`로 완료되었고, `v4_result.md`와 진단 산출물이 해당 실행 결과로 갱신되었다. 검색·독소 지표는 기존 기준값과 동일했다.
