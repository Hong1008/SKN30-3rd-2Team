...
> **이력 주의:** 아래 v3의 `CHANGED`·coverage 관련 라벨링/평가 계획은 당시 설계 기록이다. 현행 Track A 골든 계약은 `NONE`/`EXTRA`만 사용하며, 현재 이탈 성능 기준선은 v5다. 이 문서는 과거 골든 제작 이력으로 보존한다.

> 근거: [v1_review.md](../../src/eval/golden/v1_review.md) §6·§8.3(최우선 미이행 항목), [07-03 결정 로그](../decisions/07-03.md).
