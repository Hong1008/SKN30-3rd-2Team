...
sequenceDiagram
    ...
    loop 개별 사용자 조항 매칭
        ...
        review_pipe->>Core 이탈 분류: 4. NONE/EXTRA 판정
    end
    review_pipe->>Grounder 어댑터: 5. MISSING 표준조항의 관련 근거 법령 조회
...
3.  **최적 매칭 선별 (Matching 코어):** 후보군 중 점수가 가장 높은 표준 조항을 고릅니다. 이 단계는 임계값을 적용하지 않아, 낮은 점수의 근접후보도 2차 비교 근거로 보존합니다.
4.  **판정 및 누락 탐지 (Deviation 코어):** 후보가 없으면 `NO_MATCH`, 후보가 있고 점수가 매칭 임계치(기본 `0.5`) 미만이면 `EXTRA`, 이상이면 잠정 `NONE`입니다. 이어서 실제로 `NONE`으로 매칭된 표준조항이 계약 전체에서 빠졌는지 확인해 `MISSING`을 추가합니다. 본문 글자 비교·coverage 2-pass로 `CHANGED`를 반환하지 않습니다.
5.  **법령 근거 매핑 (Grounder 연동):** `MISSING` 표준조항의 카테고리를 기준으로 관련 법령 조문을 찾아 붙입니다. `NONE`/`EXTRA`의 의미 차이 설명이나 추가 근거 조회는 서버 밖 2차 LLM/클라이언트가 기존 MCP 도구를 통해 수행합니다.
6.  **위험 및 부당 패턴 추가 추적 (Graph / Toxic 코어):** 매칭된 조항의 연관 표준조항과 알려진 독소조항 패턴을 별도 검토 후보 신호로 붙입니다. 어느 신호도 위법·유불리를 단정하지 않습니다.
