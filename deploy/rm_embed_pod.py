...
def _state_id(environment: str) -> str | None:
    try:
        output = subprocess.run(["aws", "ssm", "get-parameter", "--name", f"{PREFIX.format(environment=environment)}/pod-id", "--output", "json"], check=True, capture_output=True, text=True).stdout
...
def _local_id() -> str | None:
    ...
    found = re.search(r"^RUNPOD_EMBED_POD_ID=['\"]?([A-Za-z0-9]+)", values, re.MULTILINE)
...
def _clear_local_state() -> None:
    ...
    if not path.exists():
        return
    keys = {"RUNPOD_EMBED_POD_ID", "RUNPOD_POD_BASE_URL", "RUNPOD_EMBED_API_KEY"}
...
def main() -> int:
    ...
    if args.state_backend == "aws":
        for key in ("pod-id", "base-url", "template-id", "last-provision-run-id"):
