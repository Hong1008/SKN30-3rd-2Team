...
- `pattern`, `pattern_id`, `category`, 원본 `text`는 보존한다. 리랭킹 전 복사본에
  `rerank_text`를 추가하고 `text`는 변경하지 않는다. 본문(`text`)이 없거나 문자열이 아니면
  빈 입력을 만들지 않고 명시적 예외로 중단한다.
...
4. 패턴 데이터와 색인은 변경하지 않으므로 재색인하지 않는다. 기존 인덱스가 준비됐는지만 확인하고,
   `run_eval`의 `case_ids`, `--no-write`, `--output-dir` 격리 옵션으로 **tuning 결과만 먼저** 읽는다.
   held-out 결과는 tuning 선택 기준을 통과하기 전까지 열람·해석하지 않는다.
...
- [x] 독소 후보의 **리랭커 입력에 기존 `title`을 접두**하는 코드 변경을 승인한다.
- [x] 90건 `toxic_patterns.json`과 `ToxicPattern` enum은 변경하지 않는다.
- [x] v4 tuning 기준(FN 2건 이상 감소, FP 증가 없음, F1 개선)을 승인한다.
- [x] 동결 held-out 단회 검증 기준(FN 1건 이상 감소, FP 최대 1건 증가)을 승인한다.
- [x] held-out 실패 시 추가 조정 없이 보류/폐기하고, 새 실험은 별도 사인오프로 분리한다.

승인 기록: 담당자 사용자 승인 / 승인일 2026-07-12 / 승인 범위 제목 접두 `rerank_text` 구현
