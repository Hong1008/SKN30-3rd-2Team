...
> **현재 상태(2026-07-10): 완료.** 유형별 최신판 하나만 활성화하고 과거판은 archive로 분리했으며,
> 마이그레이션 단일-version 불변식과 재평가까지 완료했다. 과거판 선택 기능은 현재 범위 밖이다.

> 근거: [07-09 결정 로그](../decisions/07-09.md) — `v3_result.md` 재평가 결과 분석 중 실측 발견.
...
## 참고
- [07-09 결정 로그](../decisions/07-09.md) — 실측 경위·재현 데이터
