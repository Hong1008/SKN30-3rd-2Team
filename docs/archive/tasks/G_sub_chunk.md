...
> **현재 상태(2026-07-10): 구현 완료.** 조건부 분할·서브청크 인덱스·부모 Max roll-up은 검색 recall
> 보강 경로에 배선됐다. 아래 `CHANGED`/커버리지 삭제 탐지는 J 이전 설계 이력이며 현재 판정 계약으로
> 그대로 구현하지 않는다. 필요하면 별도 카드에서 NONE/EXTRA 구조에 맞춰 재정의한다.
...
- [x] `vector_manager.py` Max Roll-up 검색 로직 구현 및 단위 테스트 통과
- [x] `review_pipe.py`(또는 `core/coverage.py`) 커버리지 체크 로직 구현 및 단위 테스트 통과
