...
| 커버리지 결과 | 이전(규칙계층 있음) | 이후(이 카드) |
| --- | --- | --- |
| 미달(표준 항 일부가 사용자 조항에 없음) | CHANGED (확정) | **CHANGED (확정, 변경 없음)** |
| 충족 + 치명변경 규칙 발견 | CHANGED (확정) | *(규칙 삭제로 이 경로 자체가 사라짐)* |
| 충족 + 치명변경 없음 | NONE (확정) | **NONE (잠정 — 2차 확인 전)** |
| 커버리지 체크 불가 (서브청크 미주입 등) | difflib 폴백 → NONE/CHANGED | **CHANGED (확정, 안전측)** |
...
- [ ] `Deviation.NONE` 의미 재정의가 **문서만으로 충분한지**, 아니면 정말로 "확정 vs 잠정"을 구분할
      필드(예: `provisional: bool`)가 실제 구현 중 필요해지는지 — 구현하며 재확인.
